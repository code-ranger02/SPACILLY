...
/* ─── Brand palette (theme tokens) ──────────────────────────────────────── */
const P  = 'var(--brand-primary)';
const PD = 'var(--brand-primary-hover)';
const PL = 'var(--brand-orange-text)';
      ...
      <defs>
        <pattern id="bs-grid" width="48" height="48" patternUnits="userSpaceOnUse">
          <path d="M 48 0 L 0 0 0 48" fill="none"
            stroke={isDark ? 'color-mix(in srgb, var(--brand-primary) 7%, transparent)' : 'color-mix(in srgb, var(--brand-primary) 5%, transparent)'}
            strokeWidth="1" />
        </pattern>
    ...
    <motion.div
      ...
      style={{
        background: `linear-gradient(to right, transparent, ${
          isDark ? 'color-mix(in srgb, var(--brand-primary) 30%, transparent)' : 'color-mix(in srgb, var(--brand-primary) 18%, transparent)'
        }, transparent)`,
      }}
      animate={{ top: ['8%', '92%', '8%'] }}
      ...
      >
        ...
        <div className="pointer-events-none absolute inset-0 overflow-hidden">
          <div className="absolute rounded-full" style={{
            ...
            background: isDark
              ? 'radial-gradient(circle, color-mix(in srgb, var(--brand-primary) 10%, transparent) 0%, transparent 65%)'
              : 'radial-gradient(circle, color-mix(in srgb, var(--brand-primary) 6%, transparent) 0%, transparent 65%)',
          }} />
          <div className="absolute rounded-full" style={{
            width: 420, height: 420, bottom: 80, left: -120,
            background: isDark
              ? 'radial-gradient(circle, color-mix(in srgb, var(--brand-primary-hover) 8%, transparent) 0%, transparent 65%)'
              : 'radial-gradient(circle, color-mix(in srgb, var(--brand-primary-hover) 5%, transparent) 0%, transparent 65%)',
          }} />
        </div>
        ...
        <div className="relative z-10 w-full px-4 sm:px-6 lg:px-10 xl:px-16 py-12 space-y-20">
            ...
            <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
              ...
              {reason === 'not_seller' && (
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 rounded-2xl px-6 py-5"
                  style={{
                    background: isDark ? 'color-mix(in srgb, var(--brand-primary) 8%, transparent)' : 'color-mix(in srgb, var(--brand-primary) 6%, transparent)',
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex h-11 w-11 items-center justify-center rounded-xl flex-shrink-0"
                      style={{ background: 'var(--brand-tint-strong)', color: P }}>
                      <Lock className="w-5 h-5" />
                    </div>
                    <div>
                      <p className="font-bold text-sm mb-0.5" style={{ color: 'var(--text-primary)' }}>Seller Account Required</p>
                      <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>You tried to access a seller-only page. Create your seller account to continue.</p>
                    </div>
                  </div>
              ...
              {reason === 'pending_approval' && (
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 rounded-2xl px-6 py-5"
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-bold text-sm mb-0.5" style={{ color: 'var(--text-primary)' }}>Application Under Review</p>
                      <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>Your seller application is being reviewed. Usually takes 24–48 hours.</p>
                    </div>
                  </div>
          ...
          <section ref={heroRef} className="text-center pt-4">
            <motion.div
              ...
              style={{
                background: isDark ? 'color-mix(in srgb, var(--brand-primary) 10%, transparent)' : 'color-mix(in srgb, var(--brand-primary) 8%, transparent)',
                border: '1px solid var(--brand-border-subtle)',
                color: P,
              }}
            ...
            >
              ...
              <Link
                ...
                style={{
                  background: cardBg, border: cardBorder, color: 'var(--text-primary)',
                ...
                return (
                  ...
                  >
                    <div className="absolute top-0 right-0 w-16 h-16 pointer-events-none"
                      style={{ background: `radial-gradient(circle at 80% 20%, color-mix(in srgb, ${stat.color}, transparent 87%), transparent 65%)` }} />
                    <div className="inline-flex items-center justify-center w-10 h-10 rounded-xl mb-4"
                      style={{ background: `color-mix(in srgb, ${stat.color}, transparent 91%)`, border: `1px solid color-mix(in srgb, ${stat.color}, transparent 81%)` }}>
                      <Icon size={18} style={{ color: stat.color }} />
                    </div>
                    <p className="font-black text-2xl mb-1 tabular-nums"
                      style={{
                        background: `linear-gradient(135deg, ${stat.color}, color-mix(in srgb, ${stat.color}, transparent 27%))`,
          ...
          <section>
            <div className="text-center mb-12">
              <motion.p className="text-xs font-bold tracking-[0.2em] uppercase mb-3" style={{ color: 'var(--text-muted)' }}
                initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
                How It Works
              </motion.p>
              <motion.h2 className="font-black leading-none"
                style={{ fontFamily: "'Times New Roman', Georgia, serif", fontSize: 'clamp(1.8rem, 4vw, 3rem)', letterSpacing: '-0.02em', color: 'var(--text-primary)' }}
              ...
              {STEPS.map((step, i) => {
                ...
                return (
                  <motion.div key={step.n}
                    ...
                    style={{ background: cardBg, border: cardBorder, backdropFilter: 'blur(12px)', boxShadow: cardShadow }}
                    initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }}
                    whileHover={{ y: -4, boxShadow: `0 20px 40px color-mix(in srgb, ${step.color}, transparent 87%)` }}
                  >
                    {/* watermark number */}
                    ...
                    <div className="absolute top-0 left-0 w-px h-full"
                      style={{ background: `linear-gradient(to bottom, color-mix(in srgb, ${step.color}, transparent 67%), transparent)` }} />

                    <div className="inline-flex items-center justify-center w-11 h-11 rounded-xl mb-5"
                      style={{ background: `color-mix(in srgb, ${step.color}, transparent 91%)`, border: `1px solid color-mix(in srgb, ${step.color}, transparent 81%)` }}>
                ...
                return (
                  ...
                  >
                    <div className="absolute top-0 right-0 w-24 h-24 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                      style={{ background: `radial-gradient(circle at 80% 20%, color-mix(in srgb, ${perk.color}, transparent 88%), transparent 65%)` }} />
                    <div className="inline-flex items-center justify-center w-11 h-11 rounded-xl mb-4"
                      style={{ background: `color-mix(in srgb, ${perk.color}, transparent 91%)`, border: `1px solid color-mix(in srgb, ${perk.color}, transparent 81%)` }}>
                      <Icon size={20} style={{ color: perk.color }} />
                    </div>
                    <h3 className="font-bold text-base mb-2" style={{ color: 'var(--text-primary)' }}>{perk.title}</h3>
          ...
          <motion.section
            ...
            style={{
              background: isDark
                ? 'linear-gradient(135deg, color-mix(in srgb, var(--brand-primary) 10%, transparent) 0%, color-mix(in srgb, var(--brand-primary-hover) 6%, transparent) 100%)'
                : 'linear-gradient(135deg, color-mix(in srgb, var(--brand-primary) 7%, transparent) 0%, color-mix(in srgb, var(--brand-primary-hover) 4%, transparent) 100%)',
              border: `1px solid ${isDark ? 'color-mix(in srgb, var(--brand-primary) 18%, transparent)' : 'color-mix(in srgb, var(--brand-primary) 14%, transparent)'}`,
              boxShadow: `0 0 60px ${isDark ? 'color-mix(in srgb, var(--brand-primary) 9%, transparent)' : 'color-mix(in srgb, var(--brand-primary) 5%, transparent)'}`,
            }}
            initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            <div className="absolute top-0 left-1/3 w-64 h-64 rounded-full pointer-events-none"
              style={{ background: 'radial-gradient(circle, color-mix(in srgb, var(--brand-primary) 10%, transparent), transparent 60%)' }} />
            <div className="absolute bottom-0 right-1/4 w-48 h-48 rounded-full pointer-events-none"
              style={{ background: 'radial-gradient(circle, color-mix(in srgb, var(--brand-primary-hover) 8%, transparent), transparent 60%)' }} />
